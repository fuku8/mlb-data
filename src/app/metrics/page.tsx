...
const SECTIONS: MetricSection[] = [
  ...
  },
  {
    ...
    title: "ラック指数（風向きメーター）",
    where: "選手詳細ページ（打者/投手、規定到達者のみ）",
    what: "現在の成績が実力値からどれだけ運の影響を受けているかを「追い風/向かい風」で表す。中央=実力どおり、右=追い風（実績が出来すぎ）、左=向かい風（実績が実力より悪く見えている）のメーターで示す。",
    how: "打者はStatcastのxwOBA（打球の速度・角度から算出される期待値）が取得できる選手はX版：wOBA実測−xwOBAの差を使う。±0.010未満は平年並み、±0.025以上はかなりの乖離。プラスは追い風=打球の質から期待される以上の結果が出ている（表示レンジは±0.05）。xwOBAまたはwOBA実測のどちらかが欠損する選手はBABIP版にフォールバックする：BABIP（インプレー打球の安打率）とリーグ平均（規定打者プールの平均を自算）との差。±0.015未満は平年並み、±0.040以上はかなりの乖離。プラスは追い風=打球が平均より多くヒットになっている（表示レンジは±0.06）。カードの解説文でどちらの計算かを明示する。投手はERAとFIP（三振・四球・被本塁打だけで測る「本当のERA」）の差。±0.30未満は平年並み、±0.70以上はかなりの乖離。ERAがFIPより悪い（差がプラス）と向かい風=実力より打たれて見えている。",
    population: `打者は規定打者（PA≥${HITTER_QUALIFY_PA}）、投手は規定投手（アウト数≥${PITCHER_QUALIFY_OUTS}）。X版はさらにStatcastデータ（打者フィジカルカードと同じCSV）にxwOBAが存在する選手に限られる。母集団はLeague Percentile/セイバー指標カードと共通。`,
    caveat: "X版は打球の質そのものを基準にするため、守備位置や球場の影響も含まれ乖離のすべてが運ではない。BABIP版も俊足や強い打球など実力でBABIPが高い選手がいるため同様。判定文は控えめな可能性表現にとどめている。翌シーズンには平均に近づきやすい傾向はあるが、断定はできない。",
  },
  {
    id: "quadrant",
