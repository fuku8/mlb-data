...
const HITTER_TYPE_DESC: Record<string, string> = {
  ...
  スピードスター: "らしさ: 盗塁企図数（SB+CS）と三塁打数をそれぞれリーグ内でz標準化した値の平均。どちらも大多数が0のゼロ過多分布のため、守護神・中継ぎの柱と同じく生カウントの分布で標準化している。評価点: 盗塁数・盗塁成功率・得点数の平均。",
...
const PITCHER_TYPE_DESC: Record<string, string> = {
  ...
  グラウンドボーラー: "らしさ: ゴロ/フライ比率。評価点: ゴロ/フライ比率・HR/9の低さ・ERAの低さの平均。",
};
    ...
    <div style={{ display: "grid", gap: 16 }}>
      <section className="card">
        ...
        <p style={{ margin: 0, color: "var(--muted-foreground)", fontSize: 14, lineHeight: 1.7 }}>
          ...
          +1.0以上のタイプが1つもない選手には、最もzスコアが高いタイプ1つだけを「（参考）」付きで表示する——これは「明確な適合はないが強いて言えば」という消去法の表示で、下記のリーダーボードには含まれない。ただしその最上位タイプすらリーグ平均未満（z&lt;0）の場合は、参考表示すら成立しないためバッジを表示しない。
