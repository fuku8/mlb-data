...
- [ ] `src/components/stat-radar.tsx` — 汎用5角形SVGレーダー（打者/投手で共用）
- [ ] `/players/[playerId]` に5ツールレーダー（打者）/ 投手レーダー＋総合スコア
- [ ] `src/components/total-bases-waffle.tsx` — 100マスCSS grid
- [ ] `src/lib/gini.ts` — Gini係数＋テスト、`src/components/lorenz-curve.tsx` — 純SVG
- [ ] `/teams/[teamId]` に打点依存度カード
- [ ] `src/components/quadrant-map.tsx` — 純SVG散布図（中央値クロス）
- [ ] `/players` の打撃タブに打者マップ、投球タブに投手マップ
- 検証: 同上
