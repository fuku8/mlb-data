...
| 選手一覧 | `/players` | 打撃/投球タブ切替、ソート、ページネーション、小サンプル警告、打者/投手マップ（四象限） |
| 選手詳細 | `/players/[id]` | スタッツカードグリッド（打者/投手/二刀流で自動切替）、League Percentile、5ツールレーダー/投手レーダー、塁打構成ワッフル |
| チーム詳細 | `/teams/[id]` | チーム成績 + ロスター + 選手スタッツ、Season Heartbeat、得点関与依存度・イニング依存度（Gini/Lorenz、PC時は横並び） |
| 選手比較 | `/compare` | 最大4選手の並列比較（テーブル） |
| 指標解説 | `/metrics` | feel-viz各指標の定義・計算方法・母集団・注意点の解説 |

## 技術スタック

- **Framework**: Next.js 16 (App Router, TypeScript)
- **Styling**: Tailwind CSS v4
- **Charts**: 自前SVG/純CSS（サーバーコンポーネント。チャートライブラリ不使用）
- **Icons**: Lucide React
...
├── docs/
│   └── feel-viz.md                # 指標・ビジュアル表現のプランと進捗記録
├── plan.md                        # 実装プラン
├── ui-plan.md                     # UIプラン
└── README.md
```
