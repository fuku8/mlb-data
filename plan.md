...
| 選手一覧 | `/players` | **実装済** | 打撃/投球タブ切替、ソート、ページネーション、小サンプル警告、打者/投手マップ（四象限） |
| 選手詳細 | `/players/[playerId]` | **実装済** | スタッツカードグリッド表示（打者/投手/二刀流で条件分岐）、League Percentile、レーダー、塁打構成ワッフル |
| チーム詳細 | `/teams/[teamId]` | **実装済** | ロスター + スタッツ、Season Heartbeat、得点関与依存度・イニング依存度（Gini/Lorenz） |
| 選手比較 | `/compare` | **実装済** | 最大4選手の比較テーブル |
| 指標解説 | `/metrics` | **実装済** | feel-viz 各指標の定義・計算方法・母集団・注意点（詳細は `docs/feel-viz.md`） |
...
- UI: Tailwind CSS v4
- Chart: 自前SVG/純CSS（feel-viz。Recharts は依存に残っているが未使用）
- Icons: Lucide React
